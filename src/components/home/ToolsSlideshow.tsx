    ...
    <section className="py-20 px-6 md:px-10 relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        <div className={`absolute inset-0 transition-opacity duration-1000 ${isAnimating ? 'opacity-0' : 'opacity-20'}`}>
          <div className={`absolute top-1/4 left-1/4 w-96 h-96 rounded-full filter blur-[128px] bg-gradient-to-br ${currentTool.gradient}`} />
          <div className={`absolute bottom-1/4 right-1/4 w-96 h-96 rounded-full filter blur-[128px] bg-gradient-to-br ${currentTool.gradient}`} />
        </div>
      </div>
              ...
              <div className="absolute inset-0 flex">
                {tools.map((tool, index) => (
                  ...
                  >
                    ...
                    <div className={`relative h-full flex items-center p-12 transition-opacity duration-500 ${
                      currentIndex === index ? 'opacity-100' : 'opacity-0'
                    }`}>
                      <div className="max-w-xl">
                        <h3 className={`text-4xl font-bold mb-4 bg-clip-text text-transparent bg-gradient-to-r ${tool.gradient}`}>
                          {tool.name}
                        </h3>
                        ...
                        <a
                          href="/tools"
                          className={`inline-flex items-center px-6 py-3 rounded-lg bg-gradient-to-r ${tool.gradient} text-white font-medium transition-all duration-300 hover:shadow-lg hover:shadow-current/30 transform hover:-translate-y-1`}
                        >
