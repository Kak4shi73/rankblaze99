...
exports.verifyPaymentAndCreateSubscription = exports.fixMissingSubscriptions = exports.createSubscriptionAfterPayment = void 0;
const functions = require("firebase-functions");
const admin = require("firebase-admin");
// Fix subscription creation after payment
exports.createSubscriptionAfterPayment = functions.https.onCall(async (data, context) => {
    try {
        ...
        await db.collection('subscriptions').doc(subscriptionId).set(Object.assign(Object.assign({}, subscriptionData), { createdAt: admin.firestore.FieldValue.serverTimestamp(), updatedAt: admin.firestore.FieldValue.serverTimestamp() }));
...
exports.fixMissingSubscriptions = functions.https.onCall(async (data, context) => {
    try {
        const { userId } = data;
        ...
        for (const [txnId, txnData] of Object.entries(transactions)) {
            ...
            if (subscriptionsSnapshot.exists()) {
                ...
                hasSubscriptionForTool = Object.values(subscriptions).some((sub) => sub.tools && sub.tools.some((tool) => (typeof tool === 'string' ? tool === toolId : tool.id === toolId)));
            }
            if (!hasSubscriptionForTool) {
                try {
                    // Create missing subscription
                    const result = await exports.createSubscriptionAfterPayment.run({
                        userId: txnUserId,
                        toolId,
...
exports.verifyPaymentAndCreateSubscription = functions.https.onRequest(async (req, res) => {
    // Set CORS headers
    ...
    try {
        ...
        // Create subscription
        const subscriptionResult = await exports.createSubscriptionAfterPayment.run({
            userId: finalUserId,
            toolId: finalToolId,
            merchantTransactionId: merchantTransactionId,
            amount: transactionData.amount || 0
        }, { auth: null, rawRequest: req });
        res.status(200).json({
            success: true,
...
//# sourceMappingURL=subscription-fix.js.map